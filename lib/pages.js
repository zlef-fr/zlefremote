...
// Shared icon + stylesheet links used by every secondary page (start, privacy, legal).
function sharedIconLinks() {
  return `<meta name="theme-color" content="#06060a">
<link rel="icon" href="https://assets.zlef.fr/favicon.svg" type="image/svg+xml">
<link rel="apple-touch-icon" href="https://assets.zlef.fr/apple-touch-icon.png">`;
}

// Shared stylesheet block used by every site page.
function sharedStyles() {
  return `<link rel="stylesheet" href="https://da.zlef.fr/tokens.css">
<link rel="stylesheet" href="/css/site.css?v=11">`;
}

// Shared back-nav used on secondary pages (start, privacy, legal).
function sharedBackNav(t) {
...
// Shared footer used by every site page.
function sharedFooter(t) {
  return `<footer class="foot">
  <span>${t.footer_made}</span>
  <span class="foot-links"><a href="/privacy">${t.priv_link}</a><a href="/legal">${t.legal_link || 'Legal'}</a><a href="https://zlef.fr">zlef.fr</a></span>
</footer>`;
}
...
<main>
  ...
  <section id="download" class="download">
    ...
    <p class="dl-xfce reveal">${t.dl_xfce}
      <a href="/download/zlefremote-xfce-plugin.tar.gz?v=7" download>${t.dl_xfce_cta} ${icon('arrow')}</a>
    </p>
  </section>
</main>
...
// ── Legal / Mentions légales (LCEN Art. 6) ──────────────────────────────────
const LEGAL = {
  en: {
    title: 'Legal Notice',
    sections: [
      { h: 'Publisher', p: [
        'This website (remote.zlef.fr) is published by ZLEF (zlef.fr).',
        'Contact: claude@zlef.fr',
      ] },
      { h: 'Hosting', p: [
        'The relay server is hosted on infrastructure operated by zlef.fr.',
        'For hosting enquiries contact: claude@zlef.fr',
      ] },
      { h: 'Intellectual property', p: [
        'The source code of ZlefRemote is released under the MIT License. See the repository on GitHub for details.',
      ] },
      { h: 'Limitation of liability', p: [
        'ZlefRemote is provided "as is", without warranty of any kind. The publisher cannot be held responsible for any damages arising from the use or inability to use this service.',
      ] },
    ],
  },
  fr: {
    ...
    sections: [
      { h: 'Éditeur', p: [
        'Ce site (remote.zlef.fr) est édité par ZLEF (zlef.fr).',
        'Contact : claude@zlef.fr',
      ] },
      { h: 'Hébergement', p: [
        'Le serveur relais est hébergé sur l’infrastructure exploitée par zlef.fr.',
        'Pour toute question relative à l’hébergement : claude@zlef.fr',
      ] },
      { h: 'Propriété intellectuelle', p: [
        'Le code source de ZlefRemote est publié sous licence MIT. Voir le dépôt GitHub pour les détails.',
      ] },
      { h: 'Limitation de responsabilité', p: [
        'ZlefRemote est fourni « en l’état », sans garantie d’aucune sorte. L’éditeur ne saurait être tenu responsable des dommages résultant de l’utilisation ou de l’impossibilité d’utiliser ce service.',
      ] },
    ],
  },
};
...
<link rel="canonical" href="https://remote.zlef.fr/legal">
${sharedIconLinks()}
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>${c.title} — ZlefRemote</title>
${sharedStyles()}
...
module.exports = { landing, startPage, privacyPage, legalPage, SEO, platformLinks };
