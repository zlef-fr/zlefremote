...
const DESC = {
  en: "ZlefRemote turns your phone into a wireless trackpad, keyboard and live screen for any computer — see its screen in real time and tap to control, over local Wi-Fi or end-to-end encrypted from anywhere. A small portable agent for Linux, Windows and macOS; nothing to install on the phone.",
  fr: "ZlefRemote transforme votre téléphone en trackpad, clavier et écran en direct pour n'importe quel ordinateur — voyez son écran en temps réel et touchez pour contrôler, sur Wi-Fi local ou chiffré de bout en bout, où que vous soyez. Un petit agent portable pour Linux, Windows et macOS ; rien à installer sur le téléphone.",
};
const OGT = {
...
const SEO = (lang) => `  <!--zlef-seo-->
  ...
  <meta name="keywords" content="remote mouse, phone keyboard, control computer from phone, wireless trackpad, remote desktop, screen mirroring, live screen view, tap to click, end-to-end encrypted, Linux, Windows, macOS, télécommande PC, écran à distance, souris téléphone">
...
const JSONLD = (lang) => `<script type="application/ld+json">${JSON.stringify({
  ...
  featureList: [
    ...
    'Live screen view with tap-to-click', 'Local-network mode',
    'End-to-end encrypted remote mode (AES-256-GCM)',
  ],
...
// which downloads actually exist in dist/ (so we don't advertise fakes)
function platformLinks(have, t) {
  const item = (key, label, file, sub) => {
    const ready = have.has(file);
    return { key, label, file, sub, ready };
  };
  // Windows gets the installer when it's built: it bundles the tray app AND the
  // agent, which is the whole point of the platform's front-end. The bare agent
  // stays one line below for people who want the CLI.
  const winSetup = 'zlefremote-setup-windows-amd64.exe';
  const win = have.has(winSetup)
    ? item('win', 'Windows', winSetup, t.dl_win_sub)
...
function landing(lang, have, app) {
  const t = dict(lang);
  const plats = platformLinks(have, t);
  ...
  // Desktop client: the counterpart of the phone app — drive the other machine
  // from a laptop. Only the binaries actually present in dist/ are advertised.
  const desktopFiles = [
    ['linux', 'zlefremote-desktop-linux-amd64', t.dl_linux],
    ['win', 'zlefremote-desktop-windows-amd64.exe', t.dl_win],
    ['mac', 'zlefremote-desktop-darwin-arm64', t.dl_mac],
  ].filter(([, f]) => have.has(f));
  const desktopNote = desktopFiles.length ? `<p class="dl-xfce reveal">${t.dl_desktop}
      ${desktopFiles.map(([, f, label], i) =>
        `${i ? '<span class="dl-sep"> · </span>' : ''}<a href="/download/${f}" download>${t.dl_desktop_cta} ${label} ${icon('arrow')}</a>`
      ).join('\n      ')}
    </p>` : '';

  // Windows desktop-integration note: only shown once the installer is built.
  const winNote = have.has('zlefremote-setup-windows-amd64.exe') ? `<p class="dl-xfce reveal">${t.dl_tray}
      <a href="/download/zlefremote-agent-windows-amd64.exe" download>${t.dl_tray_cli} ${icon('arrow')}</a>${
        have.has('zlefremote-windows-portable-amd64.zip')
          ? `<span class="dl-sep"> · </span><a href="/download/zlefremote-windows-portable-amd64.zip" download>${t.dl_tray_zip} ${icon('arrow')}</a>`
          : ''}
    </p>` : '';

  // ── the phone app ────────────────────────────────────────────────────────
  // The phone side used to be an installable web page. It is a native Android
  // app now, and the honest version of that sentence includes what is NOT
  // shipped: there is no iOS build, and the reason is stated rather than left
  // for the visitor to discover by looking for a button that isn't there.
  const appMeta = app
    ? t.app_meta
        .replace('{v}', app.versionName)
        .replace('{size}', `${(app.size / 1048576).toFixed(1)} MB`)
    : t.app_meta_soon;
  const appSection = `
  <section id="app" class="appsec">
    <h2 class="zl-h2 sect-title reveal">${t.app_title}</h2>
    <p class="dl-lede reveal">${t.app_b}</p>
    <div class="app-grid reveal">
      <div class="app-card">
        <div class="app-os">${icon('phone')} Android</div>
        ${app
          ? `<a class="zl-btn zl-btn--primary zl-btn--lg app-get" href="/app/zlefremote.apk" download>${icon('download')} ${t.app_get}</a>`
          : `<span class="zl-btn zl-btn--ghost zl-btn--lg app-get is-disabled" aria-disabled="true">${t.app_get}</span>`}
        <p class="app-meta">${appMeta}</p>
        <p class="app-sub">${t.app_sideload}</p>
      </div>
      <div class="app-card muted">
        <div class="app-os">${icon('phone')} iOS</div>
        <h3>${t.app_ios_t}</h3>
        <p class="app-sub">${t.app_ios_b}</p>
      </div>
      <div class="app-card muted">
        <div class="app-os">${icon('globe')} Web</div>
        <h3>${t.app_web_t}</h3>
        <p class="app-sub">${t.app_web_b}</p>
      </div>
    </div>
  </section>
`;
...
<main>
  ...
  <section id="features" class="features">
    ...
    <div class="feat-grid">
      ...
      <div class="feat reveal"><h3>${icon('monitor')} ${t.feat_5_t}</h3><p>${t.feat_5_b}</p></div>
  ...
  <section id="download" class="download">
    ...
    <p class="dl-xfce reveal">${t.dl_xfce}
      <a href="/download/zlefremote-xfce-plugin.tar.gz?v=8" download>${t.dl_xfce_cta} ${icon('arrow')}</a>
    </p>
  </section>
</main>
...
<main class="start-main">
  <section class="start-hero">
    ...
    <div class="start-copy">
      ...
      <a class="openapp" href="/app/zlefremote.apk" download>
        <span class="openapp-ic">${icon('phone')}</span>
        <span class="openapp-txt">
          <span class="openapp-t">${t.start_openapp} ${icon('download')}</span>
          <span class="openapp-s">${t.start_openapp_sub}</span>
        </span>
      </a>
      <a class="openweb" href="/r/">${t.start_openweb} ${icon('arrow')}</a>
    </div>
  </section>
</main>
...
const PRIV = {
  en: {
    ...
    sections: [
      ...
      { h: 'The Android app', p: [
        'The app holds one 256-bit key per saved computer, in the Android Keystore. Those keys never leave the phone: they are what encrypts your session, and losing them is the only thing that can un-pair a device.',
        'Permissions and why: the camera reads pairing QR codes and is used for nothing else, and no image is stored or sent; notifications carry the session card with your computer’s playback controls; a foreground service keeps the connection alive while the screen is off; “install packages” exists only so the app can install its own update, which you download from this site.',
        'The app plays no audio and takes no audio focus — the lock-screen controls are a real media session, so music playing on the phone itself is left alone.',
        'The app contains no analytics and sends no telemetry. It contacts remote.zlef.fr for exactly two things: the encrypted relay socket, and the update manifest when you check for a new version.',
        'There is no iOS version, so no data is processed on iPhones by any app of ours — an iPhone uses the web remote, covered above.',
      ] },
      { h: 'This website', p: [
  ...
  fr: {
    ...
    sections: [
      ...
      { h: 'L’application Android', p: [
        'L’app conserve une clé de 256 bits par ordinateur enregistré, dans le Keystore Android. Ces clés ne quittent jamais le téléphone : ce sont elles qui chiffrent votre session, et les perdre est la seule chose qui puisse désappairer un appareil.',
        'Les autorisations et leur raison : la caméra lit les QR codes d’appairage et rien d’autre — aucune image n’est enregistrée ni envoyée ; les notifications portent la carte de session avec les commandes de lecture de votre ordinateur ; un service de premier plan maintient la connexion écran éteint ; « installer des applications » n’existe que pour que l’app installe sa propre mise à jour, téléchargée depuis ce site.',
        'L’app ne joue aucun son et ne prend aucun focus audio : les contrôles d’écran verrouillé sont une véritable session média, la musique du téléphone n’est donc pas interrompue.',
        'L’app ne contient aucune mesure d’audience et n’envoie aucune télémétrie. Elle contacte remote.zlef.fr pour exactement deux choses : le socket chiffré du relais, et le manifeste de mise à jour quand vous cherchez une nouvelle version.',
        'Il n’existe pas de version iOS : aucune donnée n’est donc traitée sur iPhone par une de nos applications — un iPhone utilise la télécommande web, décrite plus haut.',
      ] },
      { h: 'Ce site web', p: [
